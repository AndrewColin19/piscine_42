...
/*   Updated: 2021/09/30 09:57:44 by acolin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_solve(char **tab, int size, char *c)
{
	...
	ft_set_square(&carre, 1, 0, 1);
	while (++i < size)
	{
		j = -1;
		while (++j < sizel)
		{
			if (ft_check_increment(carre, tab, c,
					ft_set_point(carre.x, carre.y, size, sizel)))
				ft_set_square(&carre, carre.x, carre.y, (carre.size + 1));
			else if (ft_check_increment(carre, tab, c,
					ft_set_point(i, j, size, sizel)))
